...
type Movie struct {
	...
	// Note that the omitempty directive will still work on this:
	// If the Runtime field has the underlying value 0, then it will
	// be considered empty and omitted --
	// and the MarshalJSON() method we just made  won't be called at all.
...
func (m MovieModel) Get(id int64) (*Movie, error) {
	...
	// so we know that no movies will have ID values less than that.
...
func (m MovieModel) Update(movie *Movie) error {
	...
	// passing in the args slice as a variadic parameter and scanning the new version value into the movie struct.
...
func (m MovieModel) Delete(id int64) error {
	...
	//	we know that the movies table didn't contain a record with the provided ID at the moment we tried to delete it.
